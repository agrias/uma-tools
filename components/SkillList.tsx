...
export function Skill(props) {
	return (
		<div class={`skill ${classnames[skilldata(props.id).rarity]} ${props.selected ? 'selected' : ''}`} data-skillid={props.id}>
			<img class="skillIcon" src={`/assets/${skillmeta(props.id).iconId}.png`} /> 
...
export function SkillList(props) {
	...
	function IconFilterButton(props) {
		return <button data-filter={props.type} class={`iconFilterButton ${active[props.group][props.type] ? 'active': ''}`} style={`background-image:url(/assets/${props.type}1.png)`}></button>
	}
